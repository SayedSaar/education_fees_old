...
frappe.ui.form.on("Student Fees", {
	setup: function(frm) {
	...
	onload: function(frm){
		...
		frm.set_query("student_fee_structure",function(){
			return{
				"filters":{
					"academic_year": (frm.doc.academic_year)
				}
	...
	academic_term: function() {
		frappe.ui.form.trigger("Student Fees", "program");
	},
...
frappe.ui.form.on("Student Fee Component", {
	amount: function(frm) {
		frm.trigger("calculate_total_amount");
	}
});
