...
class StudentFees(AccountsController):
	...
	def on_submit(self):
		...
		if self.send_payment_request and self.student_email:
			pr = make_payment_request(dt="Student Fees", dn=self.name, recipient_id=self.student_email,
...
def get_fee_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	if student:
		return frappe. db.sql('''select name, program, due_date, paid_amount, outstanding_amount, grand_total from `tabStudent Fees`
